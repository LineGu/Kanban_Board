...
export const DBcontroller = {
  async deleteContainer(idOfDeleteContainer) {
    try {
      const result = await fetch(PATH.server_url + `/user/container/${idOfDeleteContainer}`, {
        method: 'DELETE',
        credentials: 'include',
      });
  ...
  async addContainer(idOfAddContainer, name, containerIndex, userId = 0) {
    try {
      const result = await fetch(PATH.server_url + `/user/container/${idOfAddContainer}`, {
        method: 'POST',
        body: JSON.stringify({
          name,
          containerIndex,
  ...
  async updateContainer(idOfContainerUpdated, name) {
    try {
      const result = await fetch(PATH.server_url + `/user/container/${idOfContainerUpdated}`, {
        method: 'PATCH',
        body: JSON.stringify({
          name,
        }),
  ...
  async moveContainer(idOfContainerMoved, indexOfStart, indexToGo) {
    try {
      const result = await fetch(PATH.server_url + `/user/container/index/${idOfContainerMoved}`, {
        method: 'PATCH',
        body: JSON.stringify({
          indexOfStart,
          indexToGo,
  ...
  async deleteCard(idOfContainer, idOfCardToDelete) {
    try {
      const result = await fetch(
        PATH.server_url + `/user/card/${idOfContainer}/${idOfCardToDelete}`,
        {
          method: 'DELETE',
          credentials: 'include',
        },
  ...
  async addCard(idOfContainer, idOfCardToAdd, header, body, footer, cardIndex, userId = 0) {
    try {
      const result = await fetch(PATH.server_url + `/user/card/${idOfContainer}/${idOfCardToAdd}`, {
        method: 'POST',
        body: JSON.stringify({
          header,
          body,
  ...
  async updateCard(idOfContainerUpdated, idOfCardUpdated, header, body) {
    try {
      const result = await fetch(
        PATH.server_url + `/user/card/${idOfContainerUpdated}/${idOfCardUpdated}`,
        {
          method: 'PATCH',
          body: JSON.stringify({
            header,
  ...
  async moveCard(idOfContainerMoved, idOfCardMoved, indexsOfStart, indexsToGo) {
    try {
      const result = await fetch(
        PATH.server_url + `/user/card/index/${idOfContainerMoved}/${idOfCardMoved}`,
        {
          method: 'PATCH',
          body: JSON.stringify({
            indexsOfStart,
